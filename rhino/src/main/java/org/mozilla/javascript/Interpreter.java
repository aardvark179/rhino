...
public final class Interpreter extends Icode implements Evaluator {
            ...
            for (; ; ) {
                ...
                jumplessRun:
                {
                    ...
                    switch (op) {
                        ...
                        case Token.GT:
                        case Token.LT:
                            doCompare(frame, op, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.IN:
                        case Token.INSTANCEOF:
                            doInOrInstanceof(cx, op, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.EQ:
                        case Token.NE:
                            doEquals(op, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.SHEQ:
                        case Token.SHNE:
                            doShallowEquals(op, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.IFNE:
                        ...
                        case Icode_POP:
                            pop(frame.stack, state);
                            continue Loop;
                        case Icode_POP_RESULT:
                            popResult(frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Icode_DUP:
                            dup(frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Icode_DUP2:
                            dup2(frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Icode_SWAP:
                            swap(frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.RETURN:
                            doReturn(frame, frame.stack, frame.sDbl, state);
                            break Loop;
                        ...
                        case Icode_RETUNDEF:
                            ...
                            break Loop;
                        case Token.BITNOT:
                            doBitNOT(frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.BITAND:
                        case Token.BITOR:
                        case Token.BITXOR:
                        case Token.LSH:
                        case Token.RSH:
                            doBitOp(frame, op, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.URSH:
                            unsignedRightShift(frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.POS:
                            toPositive(frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.NEG:
                            toNegative(frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.ADD:
                            doAdd(frame.stack, frame.sDbl, state, cx);
                            continue Loop;
                        case Token.SUB:
                        case Token.MUL:
                        case Token.DIV:
                        case Token.MOD:
                        case Token.EXP:
                            doArithmetic(frame, op, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.NOT:
                            doNot(frame, frame.stack, state);
                            continue Loop;
                        case Token.BINDNAME:
                            doBindName(cx, frame, frame.stack, state);
                            continue Loop;
                        case Token.STRICT_SETNAME:
                        case Token.SETNAME:
                            setName(cx, frame, frame.stack, frame.sDbl, state, op);
                            continue Loop;
                        case Icode_SETCONST:
                            setConst(cx, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.DELPROP:
                        case Icode_DELNAME:
                            doDelName(cx, frame, op, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Icode_DELPROP_SUPER:
                            doDelPropSuper(frame.stack, state);
                            continue Loop;
                        case Token.GETPROPNOWARN:
                            getPropNoWarn(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.GETPROP:
                            getProp(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.GETPROP_SUPER:
                        case Token.GETPROPNOWARN_SUPER:
                            getPropSuper(cx, frame, frame.stack, state, op);
                            continue Loop;
                        case Token.SETPROP:
                            setProp(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.SETPROP_SUPER:
                            setPropSuper(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Icode_PROP_INC_DEC:
                            propIncDec(cx, frame, frame.stack, frame.sDbl, iCode, state);
                            continue Loop;
                        case Token.GETELEM:
                            doGetElem(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.GETELEM_SUPER:
                            doGetElemSuper(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.SETELEM:
                            doSetElem(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.SETELEM_SUPER:
                            doSetElemSuper(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Icode_ELEM_INC_DEC:
                            doElemIncDec(cx, frame, iCode, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.GET_REF:
                            getRef(cx, frame.stack, state);
                            continue Loop;
                        case Token.SET_REF:
                            setRef(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.DEL_REF:
                            delRef(cx, frame.stack, state);
                            continue Loop;
                        case Icode_REF_INC_DEC:
                        ...
                        case Icode_NAME_AND_THIS_OPTIONAL:
                            nameAndThisOptional(cx, frame, frame.stack, state);
                            continue Loop;
                        ...
                        case Icode_ELEM_AND_THIS:
                            elemAndThis(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Icode_ELEM_AND_THIS_OPTIONAL:
                            elemAndThisOptional(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Icode_VALUE_AND_THIS:
                            valueAndThis(cx, frame.stack, frame.sDbl, state);
                            continue Loop;
                        ...
                        case Token.ENTERWITH:
                            enterWith(cx, frame, frame.stack, frame.sDbl, state);
                            continue Loop;
                        ...
                        case Token.REF_MEMBER:
                            doRefMember(cx, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.REF_NS_MEMBER:
                            doRefNsMember(cx, frame.stack, frame.sDbl, state);
                            continue Loop;
                        case Token.REF_NAME:
                            refName(cx, frame, state, frame.stack, frame.sDbl);
                            continue Loop;
                        ...
                        case ICode_FN_STORE_HOME_OBJECT:
                            doStoreHomeObject(frame.stack, state);
                            continue Loop;
                        case Icode_CLOSURE_STMT:
    ...
    private static void doStoreHomeObject(final Object[] stack, final InterpreterState state) {
        ...
        // function]
        InterpretedFunction fun = (InterpretedFunction) stack[state.stackTop];
        Scriptable homeObject = (Scriptable) stack[state.stackTop - 4];
        fun.setHomeObject(homeObject);
    }

    private static void doScopeSave(
            CallFrame frame,
            final Object[] stack,
            final InterpreterData iData,
            final InterpreterState state) {
    ...
    private static void doScopeLoad(
            CallFrame frame,
            final Object[] stack,
            final InterpreterData iData,
            final InterpreterState state) {
        state.indexReg += iData.itsMaxVars;
        frame.scope = (Scriptable) stack[state.indexReg];
    }

    private static void doLeaveWith(CallFrame frame) {
    ...
    private static void doShortNumber(
            CallFrame frame,
            final Object[] stack,
            final double[] sDbl,
            final byte[] iCode,
        ...
        ++state.stackTop;
        stack[state.stackTop] = DOUBLE_MARK;
        sDbl[state.stackTop] = getShort(iCode, frame.pc);
        frame.pc += 2;
    }
    ...
    private static void localClear(
            final Object[] stack, final InterpreterData iData, final InterpreterState state) {
        state.indexReg += iData.itsMaxVars;
    ...
    private static void delRef(Context cx, final Object[] stack, final InterpreterState state) {
        Ref ref = (Ref) stack[state.stackTop];
        stack[state.stackTop] = ScriptRuntime.refDel(ref, cx);
    }

    private static void getRef(Context cx, final Object[] stack, final InterpreterState state) {
        Ref ref = (Ref) stack[state.stackTop];
        stack[state.stackTop] = ScriptRuntime.refGet(ref, cx);
    }

    private static void doDelPropSuper(final Object[] stack, final InterpreterState state) {
        state.stackTop -= 1;
        stack[state.stackTop] = Boolean.FALSE;
        ScriptRuntime.throwDeleteOnSuperPropertyNotAllowed();
    }

    private static void doBindName(
            Context cx, CallFrame frame, final Object[] stack, final InterpreterState state) {
        stack[++state.stackTop] = ScriptRuntime.bind(cx, frame.scope, state.stringReg);
    }

    private static void doNot(CallFrame frame, final Object[] stack, final InterpreterState state) {
        stack[state.stackTop] = !stack_boolean(frame, state.stackTop);
    }

    private static void toNegative(
            CallFrame frame,
            final Object[] stack,
            final double[] sDbl,
            final InterpreterState state) {
    ...
    private static void toPositive(
            CallFrame frame,
            final Object[] stack,
            final double[] sDbl,
            final InterpreterState state) {
    ...
    private static void pop(final Object[] stack, final InterpreterState state) {
        stack[state.stackTop] = null;
        state.stackTop--;
    }
    ...
    private static void catchScope(
        ...
        Throwable caughtException = (Throwable) stack[state.stackTop + 1];
        Scriptable lastCatchScope;
        if (!afterFirstScope) {
            lastCatchScope = null;
        } else {
            lastCatchScope = (Scriptable) stack[state.indexReg];
        }
        stack[state.indexReg] =
