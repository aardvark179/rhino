 ...
 */
final class CallNSpread extends Call {
    private final Operand[] arguments;
    private final Call.Type callType;

    CallNSpread(Operand lookupResult, Operand[] arguments, Call.Type callType) {
        super(lookupResult);
    ...
    public void interpret(Context cx, CallFrameV2 frame) {
        ...
        Object[] args1 = frame.getArguments(cx, arguments);
        var args2 = new ArrayList<>(Arrays.asList(args1));
        var spread = args2.remove(args2.size() - 1);
        spreadArray(cx, frame.scope, args2, spread);
        ...
        if (callType == Call.Type.RefCall) {
            frame.push(ScriptRuntime.callRef(fun, funThisObj, args2.toArray(), cx));
            return;
        }

        cx.lastInterpreterFrame = frame;
        frame.push(fun.call(cx, frame.scope, funThisObj, args2.toArray()));
    }
