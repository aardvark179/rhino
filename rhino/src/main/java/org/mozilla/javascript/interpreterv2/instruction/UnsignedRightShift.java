...
public class UnsignedRightShift extends Instruction {
    ...
    public void interpret(Context cx, CallFrameV2 frame) {
        ...
        if (rhs.isDouble(frame)) {
            // Narrowing is okay here because `& 0x1f` will remove high bits.
            rValue = (int) ScriptRuntime.toUint32(rhs.retrieveDouble(frame));
        } else {
        ...
        if (shouldCoerceRight) {
            // Narrowing is okay here because `& 0x1f` will remove high bits.
            rValue = (int) ScriptRuntime.toUint32(rObj);
        }

        long value = ScriptRuntime.toUint32(lValue) >>> (rValue & 0x1F);
