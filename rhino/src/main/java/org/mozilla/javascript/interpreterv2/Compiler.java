...
public class Compiler<T extends ScriptOrFn<T>> {
    ...
    public JSDescriptor<T> compile(
        ...
        builder = new CompilerData.Builder<>();
    ...
    private void generateInstructions(Node node) {
        ...
        if (CompilerData.shouldDumpInstructions) {
            builder.build().dumpInstructions(System.err, descBuilder);
        }
    }
    ...
    private void generateStatement(Node node, int initialStackDepth) {
        ...
        switch (op) {
            ...
            case Token.SWITCH:
                {
                    ...
                    var valueOperand = getOperand(child, 0, true, lineNumbers);
                    for (Jump caseNode = (Jump) child.getNext();
                        ...
                        var testOperand = getOperand(test, 0, false, lineNumbers);
                    ...
                    } else if (valueOperand instanceof PeekOperand) {
            ...
            case Token.EXPR_VOID:
                visitUnaryOperation(child, obj -> VoidInstruction.ofOperand(obj));
                return;
    ...
    private void visitExpression(Node node, int contextFlags) {
        ...
        switch (op) {
            case Token.FUNCTION:
                {
                    ...
                    addInstruction(new ClosureExpression(fnIndex, lexThisOp, homeObjOp, newTargetOp));
            ...
            case Token.NEG:
                ...
                return;
            case Token.NOT:
                visitUnaryOperation(child, obj -> Not.ofOperand(obj));
                return;
            case Token.BITNOT:
    ...
    private void addLineNumber(Node node, List<Integer> lines) {
        if (lines == null) {
            updateLineNumber(node);
            return;
        }
    ...
    private void updateLineNumbers(List<Integer> lines) {
        for (int i : lines) {
            updateLineNumber(i);
        }
    }
    ...
    private void visitIncDec(Node node, Node child) {
        ...
        switch (childType) {
            case Token.GETVAR:
                {
                    if (descBuilder.requiresActivationFrame) Kit.codeBug();
