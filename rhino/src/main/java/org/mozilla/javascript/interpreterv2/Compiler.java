...
import org.mozilla.javascript.interpreterv2.instruction.ComputedProperty;
...
import org.mozilla.javascript.interpreterv2.instruction.EmptyObjectLiteral;
...
import org.mozilla.javascript.interpreterv2.instruction.FunctionStoreHomeObject;
...
import org.mozilla.javascript.interpreterv2.instruction.LegacyInt;
import org.mozilla.javascript.interpreterv2.instruction.LegacyShort;
...
import org.mozilla.javascript.interpreterv2.operand.LegacyIntOperand;
import org.mozilla.javascript.interpreterv2.operand.LegacyOneOperand;
import org.mozilla.javascript.interpreterv2.operand.LegacyShortOperand;
import org.mozilla.javascript.interpreterv2.operand.LegacyZeroOperand;
import org.mozilla.javascript.interpreterv2.operand.NegativeZeroOperand;
...
import org.mozilla.javascript.interpreterv2.operand.ShortOperand;
...
import org.mozilla.javascript.interpreterv2.operand.ZeroOperand;
...
public class Compiler<T extends ScriptOrFn<T>> {
    ...
    private void generateNestedFunctions() {
        ...
        for (int i = 0; i != functionCount; i++) {
            ...
            // TODO(Cam): If the function can't compile, we need to use old interpreter for that
            // function only.
    ...
    private void visitExpression(Node node, int contextFlags) {
        ...
        switch (op) {
            ...
            case Token.OR:
                {
                    // TODO(Cam): Same reasoning as described in Token.NULLISH_COALESCING case
                    visitExpression(child, 0);
                    child = child.getNext();
            ...
            case Token.GT:
                {
                    // TODO(Cam): separate out into individual instructions?
                    var lhs = getOperand(child, 0);
                    child = child.getNext();
                    var rhs = getOperand(child, 0);
            ...
            case Token.SETELEM_OP:
                {
                    var lhs = getOperand(child, 0, true);
                    child = child.getNext();
                    var elem = getOperand(child, 0, true);
                    child = child.getNext();
            ...
            case Token.NUMBER:
                {
                    ...
                    if (shouldTreatNumberAsInteger(num)) {
                        if (inum == 0) {
                            ...
                            if (treatNumericLiteralsLikeOldRhino) {
                                addInstruction(new LegacyShort((short) (inum & 0xFFFF)));
                            } else {
                                addInstruction(new ShortNumber((short) (inum & 0xFFFF)));
                            }
                        } else {
                            if (treatNumericLiteralsLikeOldRhino) {
                                addInstruction(new LegacyInt(inum));
                            } else {
            ...
            case Token.NULLISH_COALESCING:
                {
                    // TODO(Cam):
                    //  In the future, we should probably use operands here. However, if the script
                    //  only contains a nullish coalescing operation, then it is possible for the
                    //  stack to be empty. Afterwards, PopResult() happens, which will fail since
                    //  the stack is empty. This is a special case because this instruction doesn't
                    //  push to the stack, similar to AND and OR.
                    visitExpression(child, 0);
                    child = child.getNext();
    ...
    private Operand getOperand(Node node, int contextFlags, boolean preferPeek) {
        switch (node.getType()) {
            case Token.NUMBER:
                {
                    ...
                    if (shouldTreatNumberAsInteger(num)) {
                        if (inum == 0) {
                            // Check for negative zero
                            if (1.0 / num < 0.0) {
                                return NegativeZeroOperand.instance;
                            } else {
                                if (treatNumericLiteralsLikeOldRhino) {
                                    return LegacyZeroOperand.instance;
                                } else {
                                    return ZeroOperand.instance;
                                }
                            }
                        } else if (inum == 1) {
                            if (treatNumericLiteralsLikeOldRhino) {
                                return LegacyOneOperand.instance;
                            } else {
                                return OneOperand.instance;
                            }
                        } else if ((short) inum == inum) {
                            if (treatNumericLiteralsLikeOldRhino) {
                                return new LegacyShortOperand((short) (inum & 0xFFFF));
                            } else {
                                return new ShortOperand((short) (inum & 0xFFFF));
                            }
                        } else {
                            if (treatNumericLiteralsLikeOldRhino) {
                                return new LegacyIntOperand(inum);
                            } else {
                                return new IntOperand(inum);
                            }
                        }
                    } else {
            ...
            // TODO(jimmy)
            // In cases like baseSlice(index, (index += 1))
            // this was causing the first index here to resolve
            // after the increment. The fix might be to change it
            // just for call. But to be on the safe side. I am removing
            // the use of these operands here all together.
            // (Same change the above to property access and the same would apply)
            // Maybe this just shows the limitation of the operand system and we
            // should only do things that can't change?
            // For right now, I'm just trying to fix things so disabling this.
            ...
            // case Token.GETPROPNOWARN:
            //     {
            //         if (node.getIntProp(Node.OPTIONAL_CHAINING, 0) == 0
            //                 && node.getIntProp(Node.SUPER_PROPERTY_ACCESS, 0) == 0) {
            //             var child = node.getFirstChild();
            //             var firstChild = child;
            //             child = child.getNext();
            //
            //             var lhs = getOperand(firstChild, 0, preferPeek);
            //             return new GetPropOperand(
            //                     lhs, child.getString(), node.getType() == Token.GETPROPNOWARN);
            //         }
            //     }
            default:
                {
    ...
    private void visitSuperIncDec(Node node, Node child, int childType, int incrDecrMask) {
        ...
        Operand one;
        if (treatNumericLiteralsLikeOldRhino) {
            one = LegacyOneOperand.instance;
        } else {
            one = OneOperand.instance;
        }
