...
public abstract class ScriptableObject extends SlotMapOwner<Scriptable>
    ...
    protected boolean defineOwnProperty(
        ...
        if (BuiltInDescriptor.isBuiltIn(aSlot)) {
    ...
    static Slot<Scriptable> setSlotValue(
        ...
        if (info.accessorDescriptor) {
            ...
            if (info.value != NOT_FOUND) {
                ...
                var bis =
                        (CompactSlot<
                                        BuiltInDescriptor<ScriptableObject>,
                                        Scriptable,
                                        ScriptableObject>)
                                slot;
                BuiltInDescriptor.setValueFromDescriptor(bis, info.value, owner, owner, true);
            }
        } else {
    ...
    protected final void checkPropertyChangeForSlot(
        ...
        if (current == null) { // new property
            ...
            if ((current.getAttributes() & PERMANENT) != 0) {
                ...
                boolean isBuiltIn = BuiltInDescriptor.isBuiltIn(current);
