...
public class AsyncParserTest {
    ...
    @Test
    public void awaitAsBinaryExpressionOperand() {
        AwaitExpression await =
                parseAndExtractAwait("async function f(t) { return t != null && await t; }");
        assertNotNull(await, "Expected AwaitExpression inside binary expression");
        assertTrue(await.getValue() instanceof Name, "Value should be a Name");
        assertEquals("t", ((Name) await.getValue()).getIdentifier());
    }

    @Test
    public void awaitBindsTighterThanAddition() {
        // Per spec AwaitExpression : await UnaryExpression, so `await a + b` is `(await a) + b`.
        AwaitExpression await =
                parseAndExtractAwait("async function f(a, b) { return await a + b; }");
        assertNotNull(await, "Expected AwaitExpression");
        assertTrue(await.getValue() instanceof Name, "Value should be a Name");
        assertEquals("a", ((Name) await.getValue()).getIdentifier());
    }
