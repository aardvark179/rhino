...
public class GeneratorStackTraceTest {
    ...
    public void testGeneratorStackTraces() {
        String script = "function* f2() { yield 1; throw 'hello'; yield 3; }; var g = f2(); g.next(); g.next();";
    ...
    private static void runWithExpectedStackTrace(
            final String _source, final String expected) {
        Utils.runWithMode(
                cx -> {
    ...
    // Doesn't work correctly in master
    @Ignore
    @Test
    public void testNestedGeneratorsException() {
        ...
        Utils.runWithMode(context -> {
            try {
                ...
                Assert.assertTrue("Stack trace should include innerGen", stack.contains("innerGen"));
                Assert.assertTrue("Stack trace should include outerGen", stack.contains("outerGen"));

                // Validate line number for the throw
                assertEquals("Line number of exception should be 4", 4, e.lineNumber());
        ...
        Utils.runWithMode(context -> {
            ...
            LambdaFunction f =
                    new LambdaFunction(
                            ...
                            "javaHelper",
                            0,
                            (Context ctx, Scriptable scope2, Scriptable thisObj, Object[] args) -> {
            ...
            try {
                ...
                Assert.assertTrue("Exception message should contain 'Java-side failure!'", e.getMessage().contains("Java-side failure!"));
